...
router.get('/', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const territoriesByUserId: Record<number, any[]> = {};
    ...
    const telephonyByUserId: Record<number, any[]> = {};
    telephonyResult.rows.forEach((tl: any) => {
...
router.put('/:id', authenticateToken, requireRole('Corporate Admin', 'Territory Admin'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (telephony && telephony.phone_number && telephony.line_type) {
      ...
      const existingTelephony = await client.query(
        'SELECT telephony_line_id FROM user_telephony_assignments WHERE user_id = $1 LIMIT 1',
        [id]
      );
      ...
      if (existingTelephony.rows.length > 0) {
        ...
        await client.query(
          `UPDATE telephony_lines SET
            phone_number = $1,
            provider = COALESCE($2, provider),
            updated_at = CURRENT_TIMESTAMP
          WHERE id = $3`,
          [telephony.phone_number, telephony.carrier, existingTelephony.rows[0].telephony_line_id]
        );
      } else {
