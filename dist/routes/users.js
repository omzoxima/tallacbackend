...
router.post('/', auth_1.authenticateToken, (0, auth_1.requireRole)('Corporate Admin', 'Territory Admin'), async (req, res) => {
    try {
        const { email, first_name, last_name, full_name, role, is_active, reports_to_id, password } = req.body;
        if (!email) {
...
router.put('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)('Corporate Admin', 'Territory Admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const { email, first_name, last_name, full_name, role, is_active, reports_to_id } = req.body;
        // Check if user exists
        const existing = await database_1.pool.query('SELECT id FROM users WHERE id = $1', [id]);
        if (existing.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        // Check if email already exists (excluding current user)
        if (email) {
            const emailCheck = await database_1.pool.query('SELECT id FROM users WHERE email = $1 AND id != $2', [email, id]);
            if (emailCheck.rows.length > 0) {
                return res.status(400).json({ error: 'Email already exists' });
            }
        }
        const computedFullName = full_name || (first_name && last_name ? `${first_name} ${last_name}` : first_name || last_name);
        const result = await database_1.pool.query(`UPDATE users SET
