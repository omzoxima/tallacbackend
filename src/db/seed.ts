...
async function seedDatabase() {
  try {
    ...
    await pool.query('DELETE FROM partner_team_members');
    await pool.query('DELETE FROM partner_territories');
    await pool.query('DELETE FROM tallac_partners');
    await pool.query('DELETE FROM territory_zip_codes');
    await pool.query('DELETE FROM territory_owners');
    await pool.query('DELETE FROM user_territory_assignments');
    await pool.query('DELETE FROM user_telephony_assignments');
    await pool.query('DELETE FROM knowledge_base_files'); // Delete before users
    await pool.query('DELETE FROM tallac_lead_contact_paths');
    await pool.query('DELETE FROM tallac_call_logs');
    await pool.query('DELETE FROM tallac_notes');
    await pool.query('DELETE FROM tallac_activities');
    await pool.query('DELETE FROM tallac_leads');
    await pool.query('DELETE FROM tallac_contacts');
    await pool.query('DELETE FROM tallac_organizations');
    ...
    await pool.query('UPDATE companies SET territory_id = NULL WHERE territory_id IS NOT NULL');
    ...
    await pool.query(`
      ...
      END $$;
    `);
    await pool.query('DELETE FROM tallac_territories');
    ...
    await pool.query('DELETE FROM telephony_lines');
    await pool.query('DELETE FROM users');
    
    console.log('✅ All data cleaned');

    // Hash password for all users
    const passwordHash = await bcrypt.hash('123@tallac', 10);
    ...
    for (const territory of territoriesData) {
      ...
      if (result.rows.length > 0) {
        ...
        const existing = await pool.query('SELECT id FROM tallac_territories WHERE territory_name = $1', [territory.name]);
    ...
    const zipCodesMap: Record<string, string[]> = {
    ...
    for (const partner of partnersData) {
      ...
      try {
        result = await pool.query(
          `INSERT INTO tallac_partners (name, partner_code, partner_name, partner_status, partner_city, partner_state, partner_email, partner_mobile, partner_address)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
           RETURNING id, name`,
          [partner.code, partner.code, partner.name, partner.status, partner.city, partner.state, partner.email, partner.mobile, partner.address]
        );
      } catch (e: any) {
        ...
        if (e.code === '23505') { // Unique violation
          ...
          result = await pool.query('SELECT id, name FROM tallac_partners WHERE partner_code = $1', [partner.code]);
        } else {
          throw e;
        }
      }
      ...
      if (result.rows.length > 0) {
        ...
        const existing = await pool.query('SELECT id, name FROM tallac_partners WHERE partner_code = $1', [partner.code]);
        if (existing.rows.length > 0) {
    ...
    const partnerTerritoryMap: Record<string, string> = {
      'Addison': 'PART-004',
      'Ann Arbor': 'PART-003',
      'Beaumont': 'PART-004',
      'Bethlehem': 'PART-001',
    ...
    for (const activity of activitiesData) {
      ...
      const prospectResult = await pool.query('SELECT organization_id FROM tallac_leads WHERE name = $1', [activity.reference]);
