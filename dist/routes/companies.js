...
router.post('/', auth_1.authenticateToken, (0, auth_1.requireRole)('Corporate Admin', 'Territory Admin'), async (req, res) => {
    try {
        const { company_name, doing_business_as, industry, status, territory_owner, mobile, address, territory_manager_email, email, map_address, full_address, location_summary, zip_code, city, state, territory_id, truck_count, driver_count, employee_count, annual_revenue, years_in_business, business_type, organization_id } = req.body;
        if (!company_name) {
            return res.status(400).json({ error: 'Company name is required' });
        }
        const result = await database_1.pool.query(`INSERT INTO companies (
...
router.put('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)('Corporate Admin', 'Territory Admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const { company_name, name, // Support name field from API
        doing_business_as, industry, industries, // Support industries field from API
        status, territory_owner, mobile, address, territory_manager_email, email, map_address, full_address, location_summary, zip_code, city, state, territory_id, truck_count, driver_count, employee_count, annual_revenue, years_in_business, business_type, organization_id } = req.body;
        ...
        const result = await database_1.pool.query(`UPDATE companies SET
...
router.put('/bulk/territory', auth_1.authenticateToken, (0, auth_1.requireRole)('Corporate Admin', 'Territory Admin'), async (req, res) => {
    try {
...
router.delete('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)('Corporate Admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const result = await database_1.pool.query('DELETE FROM companies WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Company not found' });
        }
