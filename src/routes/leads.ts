...
router.post('/bulk/assign', async (req, res) => {
  try {
    ...
    const result = await pool.query(query, [user_id, lead_ids]);
    ...
    if (result.rows.length === 0) {
      ...
      const checkResult = await pool.query(checkQuery, [lead_ids]);
...
router.post('/:id/assign', async (req, res) => {
  try {
    ...
    const result = await pool.query(query, [user_id || null, id]);
